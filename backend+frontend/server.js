...
app.get('/groups', checkForAuth('token'), async (req, res) => {
    try {
        const user = req.user; 
        const groups = await Group.find({}, '_id name description location members created_at'); // Fetch groups with members and created_at
        ...
        const userGroups = groups.map(group => {
            ...
            const formattedTime = moment(group.created_at).format('hh:mm:ss A'); // Format to show time with AM/PM
            return {
                ...group.toObject(),
                isMemberOrAdmin, 
                created_at: formattedTime // Include only the time portion with AM/PM
            };
        });
        res.status(200).send(userGroups); // Send groups with isMemberOrAdmin flag and formatted time
    } catch (error) {
